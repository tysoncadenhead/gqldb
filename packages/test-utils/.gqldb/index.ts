...
import { getAdapter, validate, getArguments, getContext, checkPermissions } from "@graphqldb/client";
...
export const getResolvers = (resolverOptions?: IGetResolvers) => ({
  ...
  Query: {
    ...
    getValidation: async (a, b) => {
      ...
      const ctx = getContext(a);
      checkPermissions([Permissions["validation.read"]], ctx, resolverOptions?.checkPermissions);
  ...
  Mutation: {
    ...
    createValidation: async (a, b) => {
      ...
      const ctx = getContext(a);
      checkPermissions([Permissions["validation.create"]], ctx, resolverOptions?.checkPermissions);
      return await Validation.create(args);
    },
    updateValidation: async (a, b) => {
      const args = getArguments<IUpdateValidation>(a, b);
      const ctx = getContext(a);
      checkPermissions([Permissions["validation.update"]], ctx, resolverOptions?.checkPermissions);
      return await Validation.update(args);
    },
    deleteValidation: async (a, b) => {
      const args = getArguments<IValidationSelectors>(a, b);
      const ctx = getContext(a);
      checkPermissions([Permissions["validation.delete"]], ctx, resolverOptions?.checkPermissions);
